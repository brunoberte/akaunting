...
const handleDeleteRecord = (row: PayableModel) => {
    if (window.confirm(`Are you sure you want to delete ${row.title}?`)) {
        try {
            router.delete(route('payables.delete', [row.id]), {
                preserveScroll: true,
                preserveState: true,
            });
...
export default function Index({
    ...
    React.useMemo(() => {
        // setIsLoading(true);
        router.visit(route('payables.index', { filter_text: filterText }), {
            preserveState: true,
            preserveScroll: true,
            onFinish: () => {
                // setIsLoading(false);
    ...
    const getVendorName = (id: number) => {
        const vendor = vendor_list.find((x) => x.id == id);
        return vendor?.name || '';
    };
    ...
    return (
        <AppLayout breadcrumbs={[{ title: pageTitle, path: route('payables.index') }]}>
                            ...
                            {record_list.map((row) => (
                                <Card key={row.id} variant="outlined">
                                    <CardContent sx={{ pb: 1 }}>
                                        ...
                                        <Grid container spacing={1}>
                                            ...
                                            <Grid item xs={6}>
                                                ...
                                                <Typography variant="body2">{getVendorName(row.vendor_id)}</Typography>
                                            </Grid>
                                            <Grid item xs={6}>
                                                <Typography variant="caption" color="text.secondary" display="block">
                                                    Category
                                    ...
                                    <CardActions sx={{ justifyContent: 'flex-end', py: 1 }}>
                                        ...
                                        <IconButton component={Link} size="small" href={route('payables.edit', { payable: row.id })} color="primary">
                                            <EditIcon />
                                        </IconButton>
