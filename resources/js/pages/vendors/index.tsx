...
export type VendorItem = z.infer<typeof schema>;
...
type VendorPagination = z.infer<typeof pagination_schema>;

const handleDeleteRecord = (row: VendorItem) => {
    if (window.confirm(`Are you sure you want to delete ${row.name}?`)) {
        router.delete(route('vendors.delete', [row.id]), {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => {
...
export default function Index({
    ...
    }, [filterText, filterEnabled, vendors.current_page]);

    const handleCreateClick = React.useCallback(() => {
        router.get(route('vendors.new'));
    }, []);

    const handleFirstPageButtonClick = () => {
        if (vendors.first_page_url) {
            router.visit(vendors.first_page_url);
        }
    };

    const handleBackButtonClick = () => {
        if (vendors.prev_page_url) {
            router.visit(vendors.prev_page_url);
        }
    };

    const handleNextButtonClick = () => {
        if (vendors.next_page_url) {
            router.visit(vendors.next_page_url);
        }
    };

    const handleLastPageButtonClick = () => {
        if (vendors.last_page_url) {
            router.visit(vendors.last_page_url);
        }
    };
    ...
    return (
        <AppLayout breadcrumbs={[{ title: pageTitle, path: route('vendors.index') }]}>
