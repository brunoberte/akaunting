...
export interface AccountFormState {
    values: Partial<Omit<ReceivableModel, 'id'>>;
...
export default function ReceivableForm({
    ...
    const { data, setData, patch, post, errors, processing, recentlySuccessful } = useForm<Required<ReceivableModel>>({
        id: receivable.id || null,
        account_id: receivable.account_id || 0,
        due_at: receivable.due_at || '',
        currency_code: receivable.currency_code || '',
        amount: receivable.amount || 0,
        title: receivable.title || '',
        customer_id: receivable.customer_id || 0,
    ...
    const pageTitle = receivable.id ? 'Edit receivable' : 'Add new receivable';

    return (
        <AppLayout breadcrumbs={[{ title: 'Receivables', path: route('receivables.index') }, { title: pageTitle }]}>
